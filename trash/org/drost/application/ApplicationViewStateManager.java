 ...
 * {@link ApplicationDataStorage} instance which may be initialized by either an
 ...
 *
 */
public class ApplicationViewStateManager 
{
...
//	private String dataDirectory = "";	// TODO Create getters and setters
	 ...
	 * {@link ApplicationViewStateManager#getStateables()} ).
	 * 
	 * @author kimschorat
	 * 
	 * @since 1.0
	...
	protected class StateGetter implements StateOperation<Map<String, Object>>
	{
		...
		public Map<String, Object> operate(Property p, Component c, Map<String, Object> stateMap) 
		{
			...
			if(!isNull(state))
			{
				String key = ApplicationViewStateManager.this.getComponentName(c);
				if(!isNull(key))
				{
	 ...
	 * 
	 * @see ApplicationViewStateManager#iterateComponentHierarchy(List, Map, StateOperation)
	 */
	protected final StateOperation<Map<String, Object>> GET_VIEW_STATE = ApplicationViewStateManager.this.new StateGetter();
	 ...
	 * @see ApplicationViewStateManager#iterateComponentHierarchy(List, Map, StateOperation)
	 */
	protected final StateOperation<Void> SET_VIEW_STATE = ApplicationViewStateManager.this.new StateSetter();
	 ...
	 * @throws IOException 
	 *            
	 * @see ApplicationDataStorage
	 */
	public ApplicationViewStateManager(String directoryPath) throws IOException
	{
		// Invalid path argument handled by the storage 
		storage = new LocalStorage(directoryPath);
	 ...
	 * {@link ApplicationDataStorage storage} to handle file storage.
	 * 
	 * @param storage
	 *            Object to handle local file storage.
	 */
	public ApplicationViewStateManager(LocalStorage storage)
	{
	 ...
	 * handle file storage.
	 * 
	 * @see ApplicationDataStorage#getSystemDefaultDirectory()
	 */
	public ApplicationViewStateManager()
	{
		try
		{
			storage = new LocalStorage(LocalStorage.getSystemDefaultDirectory());
		}
		catch ( IOException e )
		{
...
//    }
	 ...
	 *            defined by the {@link ApplicationDataStorage}.
	 */
	 ...
	 * or the user settings applied to the {@link ApplicationDataStorage}.
	 * 
	 * @param stateMap
	...
	public void restore(Component c)
	{
		if(!isNull(c))
		{
			storage.containsFile(getComponentName(c));
			// read from file
	...
	{
		boolean keysExist = storage.containsFile(componentName + STATE_KEYS_EXTENSION);
		boolean valuesExist = storage.containsFile(componentName + STATE_VALUES_EXTENSION);
